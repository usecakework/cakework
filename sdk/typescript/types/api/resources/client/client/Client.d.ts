/**
 * This file was auto-generated by Fern from our API Definition.
 */
import * as environments from "../../../../environments";
import { CakeworkApi } from "../../../..";
export declare namespace Client {
    interface Options {
        environment?: environments.CakeworkApiEnvironment | string;
    }
}
export declare class Client {
    private readonly options;
    constructor(options: Client.Options);
    /**
     * Get the status of a run
     * @throws {CakeworkApi.RunDoesNotExistError}
     */
    getRunStatus(runId: CakeworkApi.RunId, request: CakeworkApi.RunStatusRequest): Promise<CakeworkApi.Status>;
    /**
     * Get the result of a run
     * @throws {CakeworkApi.RunDoesNotExistError}
     */
    getRunResult(runId: CakeworkApi.RunId, request: CakeworkApi.RunResultRequest): Promise<CakeworkApi.RunResult | undefined>;
    /**
     * Run a task
     * @throws {CakeworkApi.RunDoesNotExistError}
     */
    run(project: CakeworkApi.Project, task: CakeworkApi.Task, request: CakeworkApi.RunRequest): Promise<CakeworkApi.RunId>;
}
